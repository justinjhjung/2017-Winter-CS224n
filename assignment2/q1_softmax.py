...
def softmax(x):
    ...
    ### YOUR CODE HERE
    # n_samples = vocab size
    # n_featurs = embedding vector size
    # eps = 1e-8
    eps = tf.constant(1e-08, tf.float32, name="epsilon")
    x -= tf.reduce_max(x)
    sum_by_row = tf.reduce_sum(tf.exp(x+eps),1)
    sum_by_row = tf.expand_dims(sum_by_row, 1)
    out = tf.exp(x+eps)/sum_by_row
    ### END YOUR CODE
