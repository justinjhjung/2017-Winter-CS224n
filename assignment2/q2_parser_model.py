...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.reshape(features, [-1, n_features*embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        ## Variables 
            # wi = index(token) of the words used in a minibatch/ dim: (m,)
            # X = [Ew1,Ew2,...Ewm] word embedding matrix/ dim: (1,dm) where d is a dimension size of a each word embedding vector
            # W = weight matrix to be multiplied with X/ dim: (dm,dh)
            # b1 = bias vector/ dim: (1,dh)
            # h = ReLu(xW + b1) hidden layer / dim: (1,dm) * (dm,dh) = (1,dh)
            # U = weight matrix to be multiplied with h/ dim: (dh,V)
            # b2 = bias vector/ dim: (1,V)
            # y_hat = softmax(hU+b2)
        
        # Dimension initialization
        m = self.config.n_features
        d = self.config.embed_size
        dh = self.config.hidden_size
        V = self.config.n_classes
        xavier_initializer = xavier_weight_init()

        # Weight matrix initialization
        W = xavier_initializer((d*m,dh))
        U = xavier_initializer((dh,V))
        W = tf.cast(W, tf.float32)
        U = tf.cast(U, tf.float32)
        b1 = tf.Variable(tf.zeros((dh)))
        b2 = tf.Variable(tf.zeros((V)))
        # Relu layer
        h = tf.nn.relu(tf.matmul(x,W)+b1)
        hdrop = tf.nn.dropout(h, self.dropout_placeholder)
        # prediction layer
