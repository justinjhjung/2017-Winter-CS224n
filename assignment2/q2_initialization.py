...
def xavier_weight_init():
    ...
    def _xavier_initializer(shape, **kwargs):
        ...
        ## Variables 
            ...
            # X = [Ew1,Ew2,...Ewm] word embedding matrix/ dim: (d,m) where d is a dimension size of a each word embedding vector
            # W = weight matrix to be multiplied with X/ dim: (m,n)
            # b = bias vector/ dim: (d,1)
            # h = ReLu(xW + b1) hidden layer / dim: (d,m) * (m,n) = (d,n)
        
        epsilon = tf.sqrt(6/(np.sum(shape)))
        out = tf.Variable(tf.random_uniform(shape=shape,minval=-epsilon, maxval=epsilon, dtype=tf.float64))
        ### END YOUR CODE
        return out
