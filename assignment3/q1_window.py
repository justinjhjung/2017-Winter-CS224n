...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        sentence = list(itertools.chain(*sentence))	# flattening the original list
        start_list = start*window_size
        end_list = end*window_size
        start_list.extend(sentence)	# appending start_list in front of the sentence
        start_list.extend(end_list)	# appending end_list at the back of the sentence
        word_flattened = start_list

        index_list = [i for i in range(int(len(sentence))) if (i%2==0)]	# get even number smaller than the length of the flattened sentence 
        for i in index_list:											 
            list_length = len(start)+2*len(start)*window_size
            windowed = word_flattened[i:i+list_length]
            label = labels[int(i/2)]
            windowed_data.append((windowed, label))
        ### END YOUR CODE
...
class WindowModel(NERModel):
    ...
    def add_embedding(self):
        ...
        lookup_table = tf.Variable(self.pretrained_embeddings) 
        embedding = tf.nn.embedding_lookup(lookup_table, self.input_placeholder)
        embeddings = tf.reshape(embedding, [-1,n_window_features*embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ## b2: (1, C)
        ## pred: (1, C)
        concat_dim = self.config.n_window_features*self.config.embed_size
        H = self.config.hidden_size
        C = self.config.n_classes

        # Variable initializing
        W = tf.get_variable("W", shape=[concat_dim, H], initializer=tf.contrib.layers.xavier_initializer())
        U = tf.get_variable("U", shape=[H, C], initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable("b1",[1,H],tf.float32,tf.constant_initializer(0))
        b2 = tf.get_variable("b2",[1,C],tf.float32,tf.constant_initializer(0))
    ...
    def add_loss_op(self, pred):
        ...
        ce_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred,labels=self.labels_placeholder)
