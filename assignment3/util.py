    ...
    for line in fstream:
        ...
        if len(line) == 0 or line.startswith("-DOCSTART-"):
            if len(current_toks) > 0:
                ...
                ret.append((current_toks, current_lbls))	# current_toks와 current_lbls를 상위의 리스트인 ret에 붙여넣음으로써 return을 만들기 시작하는 것
            current_toks, current_lbls = [], []
        else:
            assert "\t" in line, r"Invalid CONLL format; expected a '\t' in {}".format(line)
            tok, lbl = line.split("\t")	# 이 부분에서 앞서 생긴 current_toks와 current_lbls에 자료를 넣기 시작하는
            current_toks.append(tok)
            current_lbls.append(lbl)
    if len(current_toks) > 0:	# line에 해당하는 자료들을 current_toks와 lbls에 붙이고 나서 두 개 list의 길이가 같은지 확인
        assert len(current_toks) == len(current_lbls)
...
def write_conll(fstream, data):
    ...
    for cols in data:
        for row in zip(*cols):
		'''zip(*cols)의 의미:
		예) cols = [(sent1_tokens, sent1_labels),
					(sent2_tokens, sent2_labels)]
		zip(*cols) = 
		"""sent1_token1 sent1_label1
		sent1_token2 sent1_label2
		sent1_token3 sent1_label3
		...
		
		여기서는 결과적으로 tokens끼리 모은 list, labels끼리 모은 list, predictions끼리 모은 list로 만들어줌'''
