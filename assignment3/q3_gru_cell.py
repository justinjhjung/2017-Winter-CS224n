...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        z_t = sigmoid(x_t U_z + h_{t-1} W_z + b_z)  # update gate
        r_t = sigmoid(x_t U_r + h_{t-1} W_r + b_r)  # reset gate
        o_t = tanh(x_t U_o + r_t * h_{t-1} W_o + b_o)   # output gate
        ...
        with tf.variable_scope(scope):
            ...
            constant_init = tf.constant_initializer(0)
            U_r = tf.get_variable("U_r",shape=[self.input_size, self.state_size], dtype=tf.float32, initializer=xavier_init)
            W_r = tf.get_variable("W_r",shape=[self.state_size, self.state_size], dtype=tf.float32, initializer=xavier_init)
            b_r = tf.get_variable("b_r",shape=[self.state_size], dtype=tf.float32, initializer=constant_init)
            U_z = tf.get_variable("U_z",shape=[self.input_size, self.state_size], dtype=tf.float32, initializer=xavier_init)
            W_z = tf.get_variable("W_z",shape=[self.state_size, self.state_size], dtype=tf.float32, initializer=xavier_init)
            b_z = tf.get_variable("b_z",shape=[self.state_size], dtype=tf.float32, initializer=constant_init)
            U_o = tf.get_variable("U_o",shape=[self.input_size, self.state_size], dtype=tf.float32, initializer=xavier_init)
            W_o = tf.get_variable("W_o",shape=[self.state_size, self.state_size], dtype=tf.float32, initializer=xavier_init)
            b_o = tf.get_variable("b_o",shape=[self.state_size], dtype=tf.float32, initializer=constant_init)
