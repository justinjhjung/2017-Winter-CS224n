...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        # mask_temp = [True] * len(sentence)
        #
        # if len(sentence) == max_length:
        #     ret.append((sentence, labels, mask_temp))
        # elif len(sentence) < max_length:
        #     num_pad = max_length - len(sentence)  # num_pad : number of padding
        #     sentence.extend([zero_vector] * num_pad)
        #     labels.extend([zero_label] * num_pad)
        #     mask = mask_temp + num_pad * [False]
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        lookup_table = tf.nn.embedding_lookup(embed, self.input_placeholder)
        embeddings = tf.reshape(lookup_table, [-1, self.max_length, n_features*embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("layer1"):
            ...
            b2_shape = [Config.n_classes]
            U = tf.Variable(initializer(U_shape))
            b2 = tf.Variable(initializer(b2_shape))
        input_shape = tf.shape(x)
        h = tf.zeros((input_shape[0],Config.hidden_size))
        ### END YOUR CODE
        # RNN의 미니배치를 이용한 병렬처리 (SEQUENCE TAGGING WITH UNIDIRECTIONAL RNN)
        '''간단한 설명: 우리는 모든 문장에 있는 단어 sequence를 한 문장 끝나고 그 다음 문장으로 넘어가며 learning을 할 시간적 여유가 없다.
                        이 때문에 우리는 병렬처리를 통한 효율 증대에 관심이 있는 것이다. 그럼 그것을 어떻게 할 것인가? 우선 몇 가지 용어에 대한 정리부터 해야한다.
                        <데이터의 위계구조>
                        데이터(모든 문장) > 각 문장 > 각 단어 > 단어를 이루는 feature
                        <효율적인 문장 처리>
                        1. 문장의 최대 길이를 정한 다음 그것보다 긴 문장은 자르고, 짧은 문장은 padding을 하여 같은 길이로 맞추고 그것을 max_length로 규정한다. 
                            - 최초 input: (dim) (문장 갯수, 문장 최대길이(max_length), n_word_features)
                        2. 각 단어를 나타내기 위해 윈도우를 사용한다. 윈도우 사이즈를 1이라고 했을 때 각 단어는 3개의 단어(n_features)로 나타내어진다.
                            - input_placeholder: (dim) (문장 갯수, 문장 최대길이(max_length), n_features)
                            where n_features = (2*window_size+1)*n_word_features
                        3. 한 단어를 나타내기 위해 만들어진 맥락으로서 윈도우 내부의 단어들 각각에 대한 word embedding vector를 일렬로 나열한다. (reshaping)
                            - x: (dim) (문장 갯수, 문장 최대길이(max_length), n_features*embed_size)
                        4. max_length는 통일된 각 문장의 길이로서 time-step의 총 길이를 나타낸다. 우리는 문장을 한 개씩 차례대로 학습하는 것이 아니라,
                        각 문장의 첫 단어 vector들(길이: n_features*embed_size)을 학습하고 그것들을 RNNCell에 넣고, 그 다음 단어들로 동시에 진행하도록 해서
                        마지막 time-step이 되면 학습이 끝나고 parameter들이 완성되도록 할 수 있다.'''
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):  # http://dirko.github.io/Bidirectional-LSTMs-with-Keras/ 에서 time-step별 병렬처리 참고

                ### YOUR CODE HERE (~6-10 lines)
                if time_step > 0:
                    tf.get_variable_scope().reuse_variables()
                _, h = cell(x[:,time_step,:],h, scope="RNN")
                dropout_h = tf.nn.dropout(h, dropout_rate)
