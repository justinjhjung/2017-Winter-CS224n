...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):        # vanishing gradient 문제에 대응하기 위해 매 step마다 gradient를 normalize해주면 좋다는 결론이 나 있음 (gradient normalization)
        ...
        grads_and_vars = optimizer.compute_gradients(loss)   # grads : gradients, vars : variables
        grads, vars = zip(*grads_and_vars)                  # grads_and_vars는 원래 [(grad1,var1),(grad2,var2)...]이렇게 구성되어 있는데,
                                                            # grad는 grad끼리, var는 var끼리 묶어서 저장하고 싶을 때, zip(*)을 쓰면 된다.
        if self.config.clip_gradients:  # 질문: self.config.clip_gradients는 어디서 나온 건가? 원래 없었는데 ㅠ
